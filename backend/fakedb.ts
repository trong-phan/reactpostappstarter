...
export const updatePost = (id: number, post: any) => {
  let updatingPostIndex = posts.findIndex((p) => p.id === id);
  if (updatingPostIndex < 0) throw new Error('Post not found');

  posts[updatingPostIndex]['title'] = post.title;
  posts[updatingPostIndex]['category'] = post.category;
  posts[updatingPostIndex]['content'] = post.content;
  posts[updatingPostIndex]['image'] = post.image;

  return posts[updatingPostIndex];
};
